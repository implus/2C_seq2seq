...
function make_embed(data, opt)
    local input_size = opt.word_vec_size
    local src = data.source_size local src_sqrt = math.ceil(math.sqrt(src))
    local trg = data.target_size local trg_sqrt = math.ceil(math.sqrt(trg))
    return nn.LookupTable(src_sqrt, input_size), nn.LookupTable(src_sqrt, input_size),
           nn.LookupTable(trg_sqrt, input_size), nn.LookupTable(trg_sqrt, input_size)
end
...
function make_lstm(data, opt, model)
    ...
    local inputs = {}
    table.insert(inputs, nn.Identity()()) -- x (batch_size x max_word_l) -> (batch_size x word_embedding)
    if model == 'dec' then
...
function make_decoder_attn(data, opt, simple)
    ...
    -- attention! source_l is actually source_l*2 for 2C-seq2seq
...
function make_generator(data, opt)
    local trg = data.target_size 
    local trg_sqrt = math.ceil(math.sqrt(trg))
    local modelx = nn.Sequential()
    modelx:add(nn.Linear(opt.rnn_size, trg_sqrt))
    modelx:add(nn.LogSoftMax())
    local modely = nn.Sequential()
    modely:add(nn.Linear(opt.rnn_size, trg_sqrt))
    modely:add(nn.LogSoftMax())

    criterion = nn.ClassNLLCriterion()
    criterion.sizeAverage = false
    return modelx, modely, criterion
end
