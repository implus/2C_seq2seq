...
function updatebest_c(data, round)
    ...
    --for i = 1, 10 do
        ...
        total = total + nonzeros
        -- {encoder, decoder, generatorx, generatory, enc_embedx, enc_embedy, dec_embedx, dec_embedy, encoder_bwd}
        if i % math.floor((data:size() / 100)) == 1 then
            print(i, '/', data:size())
        end
    end
    local valid = math.exp(nll/total)
    ...
    local cmd = './implus '..xfilename..' '..yfilename..' '..mapxyfilename..' '.. opt.trg_idx2word_path
    print(cmd)
...
function main()
    ...
    if opt.train_from:len() == 0 then
        ...
        opt.src_mapx  = model_opt.src_mapx
        opt.src_mapy  = model_opt.src_mapy
        opt.trg_mapx  = model_opt.trg_mapx
        opt.trg_mapy  = model_opt.trg_mapy
        --opt.src_mapx   = nil
        --opt.trg_mapx   = nil
