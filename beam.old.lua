...
function generate_beam(model, initial, K, max_sent_l, source, gold)
    ...
    while (not done) and (i < n) do
        ...
        for k = 1, K do
            State.disallow(out_float:select(1, k)) -- let 'pad' and 'eos' symbol become no choice
        ...
        if model_opt.attn == 1 then end_attn_argmax = attn_argmax[i][1] end       
        if end_hyp[#end_hyp] == END then done = true found_eos = true
        else
            for k = 1, K do
